...
// Read today's date lazily (per assertion) so a date rollover during a long
// test run doesn't break expectations baked at module load time.
...
describe('toggleTaskCompletionById', () => {
  ...
  it('clears completion when toggled while already completed today', () => {
    const next = toggleTaskCompletionById(
      [makeTask({ id: 'a', completedDate: today() })],
      'a',
    )
    expect(next[0]?.completedDate).toBeNull()
  })

  it('cascades the completion state to all descendants (down-only)', () => {
    const tasks: Task[] = [
      makeTask({
        id: 'a',
        tasks: [makeTask({ id: 'b', tasks: [makeTask({ id: 'c' })] })],
      }),
    ]
  ...
  it('does not propagate upward: completing the last child leaves the parent unchanged', () => {
    const tasks: Task[] = [
      makeTask({ id: 'parent', tasks: [makeTask({ id: 'only-child' })] }),
    ]
    const next = toggleTaskCompletionById(tasks, 'only-child')
    expect(next[0]?.completedDate).toBeNull()
    expect(next[0]?.tasks[0]?.completedDate).toBe(today())
  })
})
...
describe('toggleTaskOpenById', () => {
  it('flips a nested task open state', () => {
    const tasks: Task[] = [
      makeTask({ id: 'a', tasks: [makeTask({ id: 'b', isOpen: true })] }),
    ]
...
describe('setTaskNameById', () => {
  it('renames a task by id (port-local helper used by the store)', () => {
    const tasks: Task[] = [
      makeTask({ id: 'a', name: 'old', tasks: [makeTask({ id: 'b', name: 'unchanged' })] }),
    ]
    const next = setTaskNameById(tasks, 'a', 'new')
    expect(next[0]?.name).toBe('new')
    expect(next[0]?.tasks[0]?.name).toBe('unchanged')
  })
})
