/**
 * Pure task-tree operations. Ported byte-for-byte from
 *   client-web-old/src/app/features/tasks/data-access/tasks.tree.ts
 *
 * All functions are immutable: they take a `Task[]` and return a new `Task[]`.
 * No Solid imports here — these functions are pure JS, tested in isolation,
 * and reused both by the tasks store and by the alarms/timers schedulers.
 */
...
export const toggleTaskOpenById = (
  tasks: Task[],
  taskId: string,
  isOpen: boolean,
): Task[] =>
...
export const setTaskNameById = (
  ...
  tasks.map((task) => {
    ...
    return { ...task, tasks: setTaskNameById(task.tasks, taskId, name) }
  })
